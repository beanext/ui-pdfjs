  ...
  function $PDFViewer($interval) {
    return {
      ...
      link: function ($scope, $element, $attrs) {
        ...
        if ($scope.onePage) {
          angular.element("#viewerContainer").css({overflow: 'hidden'});
        }
        ...
        function reset() {
          ...
          if ($scope.width !== undefined) {
            outerContainerStyle.width = $scope.width;
          }
          if ($scope.height !== undefined) {
            outerContainerStyle.height = "calc(" + $scope.height + " + 26px)";
          }
          angular.element("#outerContainer").css(outerContainerStyle);
        }

        window.addEventListener('updateviewarea', function (evt) {
          var pdfViewer = PDFViewerApplication.pdfViewer;
          if (pdfViewer._pages.length) {
            $scope.height = document.querySelectorAll('.page')[0].style.height;
            reset();
          }
        })
        function onPdfInit() {
